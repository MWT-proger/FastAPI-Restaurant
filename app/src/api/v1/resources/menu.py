...
from api.v1.schemas.menu import (
    MenuCreate,
    MenuResponse,
    MenuUpdate,
    delete_menu_schema,
...
from services.menu import MenuService, get_menu_service
...
async def create_menu(
    menu_content: MenuCreate = Body(None, examples=MenuCreate.Config.json_schema_extra["examples"]),
    menu_service: MenuService = Depends(get_menu_service),
) -> MenuResponse:
    """Создает новое меню.
...
async def patch_menu(
    menu_id: uuid.UUID,
    menu_content: MenuUpdate = Body(None, examples=MenuUpdate.Config.json_schema_extra["examples"]),
    menu_service: MenuService = Depends(get_menu_service),
) -> MenuResponse:
    """Обновляет меню.
