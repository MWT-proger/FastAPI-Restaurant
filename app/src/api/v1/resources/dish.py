...
from api.v1.schemas.dish import (
    DishCreate,
    DishResponse,
    DishUpdate,
    delete_dish_schema,
...
from services.dish import DishService, get_dish_service
...
async def create_dish(
    menu_id: uuid.UUID,
    submenu_id: uuid.UUID,
    dish_content: DishCreate = Body(None, examples=DishCreate.Config.json_schema_extra["examples"]),
    dish_service: DishService = Depends(get_dish_service),
) -> DishResponse:
...
async def patch_dish(
    ...
    submenu_id: uuid.UUID,
    dish_id: uuid.UUID,
    dish_content: DishUpdate = Body(None, examples=DishUpdate.Config.json_schema_extra["examples"]),
    dish_service: DishService = Depends(get_dish_service),
) -> DishResponse:
